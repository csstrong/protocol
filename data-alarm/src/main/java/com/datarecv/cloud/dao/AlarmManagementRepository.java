...
import com.datarecv.cloud.dto.BaseAlarmManagementDto;
...
public class AlarmManagementRepository {
    ...
    public Map<String, BaseAlarmManagementDto> findById(String id) {
        ...
        query.addCriteria(Criteria.where(Constant.TYPE).in("0","1","2"));
  ...
  public void replace(BaseAlarmManagementDto baseAlarmManagementDto){
         Query query=Query.query(Criteria.where("SiteId").is(baseAlarmManagementDto.getSiteId()));
         query.addCriteria(Criteria.where("IsCloggingAlarm").is(baseAlarmManagementDto.isCloggingAlarm()));
         query.addCriteria(Criteria.where("AlarmOff").is("0"));
         List<BaseAlarmManagementDto> list= this.mongoTemplate.find(query,BaseAlarmManagementDto.class,Constant.BASE_ALARM_MANAGEMENT);
         if (list.size()<=0){
             mongoTemplate.save(baseAlarmManagementDto,Constant.BASE_ALARM_MANAGEMENT);
         }
  }
}
